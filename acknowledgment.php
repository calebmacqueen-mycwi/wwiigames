<?php
    ...
    $visitor_msg = filter_input(INPUT_POST, 'comments');
    $employeeRefNum = 1;
    $easyError = '';
    /* echo "Fields: " . $visitor_name . $visitor_email . $visitor_msg;  */
    
    // Validate inputs
    if ($visitor_email == null || $visitor_msg == null) {
        ...
        exit();
            ...
            try {
                ...
                $easyError = 'I\'m sorry, the database is unavailable at this time';
                $error = $e->getMessage();
                /* include('database_error.php'); */
                include 'error.php'; 
                exit();
            }
            $query = 'INSERT INTO recordcomments
                         (employeeRefNum, emailAddress, commentTxt)
                      VALUES
                         (:employeeRefNum, :visitor_email, :visitor_msg)';
            $statement = $db->prepare($query);
            $statement->bindValue(':visitor_email', $visitor_email);
            $statement->bindValue(':visitor_msg', $visitor_msg);
            $statement->bindValue(':employeeRefNum', $employeeRefNum);
            $statement->execute();
    ...
    <div class="container">
      ...
      <main>
        <div class="row">
          ...
          <div class="col-md-10">
              <p>Thanks for you comment! It has been passed on to our event planners. Our response will be sent to <?php echo"$visitor_email";?></p>
          </div>
          <div class="col-md-1">
          </div>
        </div>
